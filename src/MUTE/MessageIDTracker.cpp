 ...
 * Added support for tracking counter rollovers.
 */
...
MessageIDTracker::MessageIDTracker( int inCacheSize )
    ...
    int logMessageHistory = settings->getLogMessageHistorySetting();
            ...
            if( ! mListHead->nameExists( uniqueName ) ) {
                ...
                if( mCurrentCacheSize < mMaxCacheSize ) {
                    ...
                    if( counter <= mListHead->mCounter ) {
                        ...
                        if( isNewTail ) {
                            // last item in list is not necessarily freshest
                            // counter, since we may have sent messages
                            // out with even fresher counters
                            if( counter > mFreshestCounter ) {
                    ...
                    if( belowFreshest ||
                        ...
                        if( belowHead ) {
                            ...
                            if( isNewTail ) {
                                if( !belowFreshest ) {
                                    // even fresher than our freshest
                                    mFreshestCounter = counter;
                                    }
                                }
...
unsigned int MessageIDTracker::getFreshCounter() {
    ...
    // update our freshest counter
    mFreshestCounter = newID;
...
char MessageIDTracker::hasCounterRolledOver() {
    mLock->lock();
