...
/**
 * Maps a virtual address to a human-friendly mnemonic.
 *
 * @param inVirtualAddress the address to map to a mnemonic.
 *
...
/**
 * Sets the key that this node will use to sign all outbound messages.
 *
 * Note that the SHA1 hash of inPublicKey should be used as this
 * node's "from" address if signing keys are set.  See
 * MUTE/doc/messageFormat.txt
 *
 * @param inPublicKey the DER-encoded, hex-encoded RSA public key, or NULL
 *   to disable signatures (default).
 *   Destroyed by caller.
 * @param inPrivateKey the DER-encoded, hex-encoded RSA private key, or NULL
 *   to disable signatures (default).
 *   Destroyed by caller.
 */
...
/**
 * Returns true if global message counter has wrapped around.
 *
 * This means that replay attacks of our signed messages are now possible (if
 * an attacker saved signed messages that we sent last time through the counter
 * space, the attacker can start sending them out now).
 *
 * A new keypair for signing messages (and thus a new PKH virtual address)
 * should be generated whenever the counter wraps around.
 *
 * @return true if the counter wrapped around (since the last call to this
 *   function), or false if not.
 */
 ...
 *
 * @return true to drop all messages, or false to pass messages on.
 */
