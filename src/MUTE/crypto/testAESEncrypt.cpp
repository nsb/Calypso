...
void AESEncryptFile( const char *keyFilename,
    ...
    CFB_Mode_ExternalCipher::Encryption cfbEncryption( aesEncryption,
                                                       (byte *)iv );
	...
	if( messageFile != NULL && outputFile != NULL ) {
		...
		while( charReadAsInt != EOF ) {
            ...
            cfbEncryption.ProcessString( (byte *)oneCharOutputString,
...
int main( char inNumArgs, char **inArgs ) {
	...
	AESEncryptFile( inArgs[1], inArgs[2], inArgs[3] );
