                        ...
                        const char *message )
{
    ...
    if (strlen(message) > pub.FixedMaxPlaintextLength())
    {
    ...
    char *outstr = new char[2*pub.FixedCiphertextLength()+1];
    pub.Encrypt( randPool, (byte *)message, strlen(message), (byte *)outstr );

    HexEncoder hexEncoder;
    hexEncoder.Put((byte *)outstr, pub.FixedCiphertextLength());
    hexEncoder.MessageEnd();
    hexEncoder.Get((byte *)outstr, 2*pub.FixedCiphertextLength());

    outstr[2*pub.FixedCiphertextLength()] = '\0';
    return outstr;
}
