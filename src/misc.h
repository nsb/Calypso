...
//     }
    ...
    static KommuteDefs::FileType fileType(QString ext)
    {
        KommuteDefs::FileType fileType = KommuteDefs::UNKNOWN;
        ext = ext.toLower();

        if (ext == "jpg" || ext == "jpeg" || ext == "png" || ext == "gif" || ext == "tif" || ext == "tiff"
            || ext == "bmp" || ext == "ico" || ext == "xpm" || ext == "svg")
        {
            fileType = KommuteDefs::PICTURE;
        }
        ...
        {
            fileType = KommuteDefs::VIDEO;
        }
        else if (ext == "ogg" || ext == "mp3" || ext == "mp1" || ext == "mp2"|| ext == "ram" || ext == "wav" || ext == "wma")
        {
            fileType = KommuteDefs::AUDIO;
        }
