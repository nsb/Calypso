...
extern qulonglong TotBytesEmitted;
...
OutboundChannel::OutboundChannel( OutputStream *inOutputStream,
    ...
    : mLock( new QMutex() ), mMessageReadySemaphore( new Semaphore() ),
...
void OutboundChannel::run() {
    ...
    while( !stopped ) {
        ...
        if( message != NULL ) {
            ...
            mLimiter->bytesTransmitted(strlen(message));
                        
            int bytesSent = mStream->writeString( message );
	    if ( bytesSent >0 )
	        TotBytesEmitted += bytesSent;
