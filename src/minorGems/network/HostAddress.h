...
class HostAddress : public Serializable {
	...
	public:
		 ...
		 * statistics 
		 */
		int mOkConnections; // nb of connections initiated by ourself.
		int mOkPing; 
		QDateTime created;
		QDateTime lastTry;
		QDateTime lastPing;
...
inline void HostAddress::initStats() {
	mOkConnections=0;
	mOkPing=0;
	created = QDateTime::currentDateTime ();
	lastTry.setTime_t(0);
	lastPing.setTime_t(0);
	lastConnection.setTime_t(0);
	score=0.0;
}
...
inline HostAddress *HostAddress::copy() {
	...
	strcpy( stringCopy, mAddressString );
        HostAddress *ret=new HostAddress( stringCopy, mPort );
	ret->mOkConnections = mOkConnections;
	ret->mOkPing = mOkPing;
	ret->created = created;
	ret->lastTry = lastTry;
	ret->lastPing = lastPing;
	ret->lastConnection = lastConnection;
	ret->score = score;
	return ret;
	}
